...
# Set up cache folder
cache_folder = os.path.join(os.path.expanduser("~"), "Desktop", "cache_folder")
...
def visualize_data(df):
    # Filter to only include 2025 Data
    ...
    # Get average times for each driver in each session
    q1_times = [df_2025[df_2025['Driver'] == driver]['Q1_sec'].mean() for driver in drivers]
    q2_times = [df_2025[df_2025['Driver'] == driver]['Q2_sec'].mean() for driver in drivers]
    q3_times = [df_2025[df_2025['Driver'] == driver]['Q3_sec'].mean() for driver in drivers]
    
    plt.bar(x - width, q1_times, width, label='Q1')
    plt.bar(x, q2_times, width, label='Q2')
    plt.bar(x + width, q3_times, width, label='Q3')

    plt.title('Average Qualifying Times by Driver')
    plt.xlabel('Driver')
    plt.ylabel('Time (seconds)')
    plt.xticks(x, drivers, rotation=45, ha='right')

    # Add gridlines to make it easier to see the differences
...
def fetch_recent_data():
    ...
    current_year = 2025
    for round_num in range(1, 3):  # First races of 2025
...
def apply_performance_factors(predictions_df):
    ...
    team_factors = {
        ...
        'Red Bull Racing': 0.998,         # -0.15s from base
    ...
    driver_factors = {
        ...
        'Oscar Piastri': 0.999,      # Base time (strong)
        ...
        'Kimi Antonelli': 1.000,     # +0.0s
        'Oliver Bearman': 1.001,     # +0.1s
        'Yuki Tsunoda': 1.002,       # +0.2s
        'Isack Hadjar': 1.002,       # +0.2s
        'Jack Doohan': 1.002,        # +0.2s
        'Gabriel Bortoleto': 1.003,  # +0.3s
        'Nico Hulkenberg': 1.003,    # +0.3s
        'Liam Lawson': 1.004,        # +0.4s
...
def predict_japanese_gp(model, latest_data):
    ...
    # Define team colors for better visualization
    team_colors = {
        ...
        'Haas F1 Team': '#808080',     # White/gray
        'Kick Sauber': '#32CD32',      # Lime Green
        'Alpine': '#0090FF'            # Bright blue
    }
    
    # Add position axis (1st, 2nd, etc.)
    positions = np.arange(1, len(results_df) + 1)
    
    # Create horizontal bar chart
    bars = plt.barh(positions, results_df['Predicted_Q3'], color=[team_colors.get(team, '#888888') for team in results_df['Team']])
    
    # Add driver names and teams as labels
    for i, (idx, row) in enumerate(results_df.iterrows()):
        plt.text(row['Predicted_Q3'] - 0.3, positions[i], f"{row['Driver']} ({row['Team']})", 
                 va='center', ha='right', color='white', fontweight='bold')
        plt.text(row['Predicted_Q3'] + 0.05, positions[i], f"{row['Predicted_Q3']:.3f}s", 
                 va='center', ha='left')
    
    # Create a custom legend for teams
    from matplotlib.patches import Patch
    legend_elements = [Patch(facecolor=color, label=team) for team, color in team_colors.items()]
    plt.legend(handles=legend_elements, loc='upper right')
    
    # Set chart properties
    plt.title('Japanese GP 2025 - Predicted Q3 Times', fontsize=16)
    plt.xlabel('Lap Time (seconds)')
    plt.ylabel('Grid Position')
    
    # Set x-axis limits to focus on the small time differences
    min_time = results_df['Predicted_Q3'].min() - 1
    max_time = results_df['Predicted_Q3'].max() + 0.3
    plt.xlim(min_time, max_time)
    
    # Invert y-axis so that P1 is at the top
...
if __name__ == "__main__":
    ...
    if all_data:
        ...
        # Filter to only include 2025 data
        combined_df_2025 = combined_df[combined_df['Year']==2025].copy()
  
        valid_data = combined_df_2025.dropna(subset=['Q1_sec', 'Q2_sec', 'Q3_sec'], how='all')

        visualize_data(valid_data)
